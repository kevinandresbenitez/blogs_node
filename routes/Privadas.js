...
aplicacion.get("/votar_publicacion" , function(ped,res){
    ...
    if (!ped.session.usuario) {
    res.redirect("/ingresar");
    }
    else {
            coneccion.getConnection(function (error, conex) {
            var query = `select * from publicaciones where id=${conex.escape(ped.query.id)} and autor_id = ${conex.escape(ped.session.usuario.id)};`
...
aplicacion.get("/admin/index", function (ped, res) {
    if (!ped.session.usuario) {
        ...
        coneccion.getConnection(function (eror, conex) {
            var query = `select titulo,resumen,contenido,foto,votos,fecha_hora,email,contrasena,pseudonimo,avatar,publicaciones.id from publicaciones right join autores on publicaciones.autor_id=autores.id where email = ${conex.escape(ped.session.usuario.email)} and contrasena = ${conex.escape(ped.session.usuario.contrasena)};`
            coneccion.query(query, function (error, lista, columnas) {
                res.render("admin/admin_index.ejs", { ingreso_registro: ped.flash("correcto"), datos: lista,usuario :ped.session.usuario });
            })
            conex.release()
        })
    }
...
aplicacion.get("/agregar", function (ped, res) {
    if (!ped.session.usuario) {
        ...
        res.render("admin/agregar.ejs",{usuario :ped.session.usuario});
    }
})
aplicacion.post("/procesar_agregar", function (ped, res) {
    if (!ped.session.usuario) {
        ...
        var votos = 0;
        var id_usuario = ped.session.usuario.id;
...
aplicacion.get("/eliminar_public", function (ped, res) {
    ...
    if (!ped.session.usuario) {
        ...
        coneccion.getConnection(function (erro, conex) {
            var query = `delete from publicaciones where id=${conex.escape(ped.query.id_publ)} and autor_id = ${conex.escape(ped.session.usuario.id)} ; `;
...
aplicacion.get("/editar_publ", function (ped, res) {
    ...
    if (!ped.session.usuario) {
        ...
        coneccion.getConnection(function (error, conex) {
            var query = `select * from publicaciones where id=${conex.escape(ped.query.id_publ)} and autor_id = ${conex.escape(ped.session.usuario.id)};`
            coneccion.query(query, function (eror, listas, columnas) {

                if (listas.length > 0) {
                    res.render("admin/editar_public.ejs", { publicacion_id: ped.query.id_publ, datos:listas[0],usuario :ped.session.usuario })
                }
                else {
                    res.redirect("/ingresar");
...
aplicacion.post("/procesar_editar", function (ped, res) {
    ...
    if (!ped.session.usuario) {
        ...
        coneccion.getConnection(function (error, conex) {
            var query = `update publicaciones set titulo=${conex.escape(titulo)},resumen=${conex.escape(resumen)},contenido=${conex.escape(contenido)} where id=${conex.escape(id_publicacion)} and autor_id = ${conex.escape(ped.session.usuario.id)};`
